...
to_pil_image(mask).save("dog_mask_deeplabv3.png")
...
# Save the mask as a grayscale image
to_pil_image(mask).save("cow_mask_deeplabv3.png")
...
for i in range(0, len(video), 1):
    ...
    to_pil_image(mask).save(f"mask_deeplabv3/cow_mask_{i}.png")
...
img = cv2.imread("mask_deeplabv3/cow_mask_0.png")
height, width, layers = img.shape

# get number of frames
len_video = len(os.listdir("mask_deeplabv3/"))

# Define the codec and create VideoWriter object
fourcc = cv2.VideoWriter_fourcc(*"mp4v")
out = cv2.VideoWriter("cow_mask_deeplabv3.mp4", fourcc, 24.0, (width, height))

for i in range(len_video):
    img = cv2.imread(f"mask_deeplabv3/cow_mask_{i}.png")
    out.write(img)
