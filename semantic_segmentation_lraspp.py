...
to_pil_image(mask).save("dog_mask_lraspp.png")
...
# Save the mask as a grayscale image
to_pil_image(mask).save("cow_mask_lraspp.png")
...
for i in range(0, len(video), 1):
    ...
    to_pil_image(mask).save(f"mask_lraspp/cow_mask_{i}.png")
...
img = cv2.imread("mask_lraspp/cow_mask_0.png")
height, width, layers = img.shape

# get number of frames
len_video = len(os.listdir("mask_lraspp/"))

# Define the codec and create VideoWriter object
fourcc = cv2.VideoWriter_fourcc(*"mp4v")
out = cv2.VideoWriter("cow_mask_lraspp.mp4", fourcc, 24.0, (width, height))

for i in range(len_video):
    img = cv2.imread(f"mask_lraspp/cow_mask_{i}.png")
    out.write(img)
